...
class HrSalaryRule(models.Model):
    ...
    def _auxtransp(self, ld):
        res = False
        deducciones = 0
        work102 = ld['worked_days'].dict['WORK102'].number_of_days
        dias = 0
        e_v = self.env['variables.economicas']
        ...
        if company.aux_trans_second:
            if ((bmt == 'second' or sch_pay == 'monthly') and ncod == 'Nomina') or (ncod in ('Liquidacion', 'Vacaciones')):
                respol = True
        else:
            if ncod in ('Liquidacion', 'Vacaciones', 'Nomina'):
                respol = True
